"""
Imovirtual.com adapter — second-largest Portuguese property portal.
...
class ImovirtualComAdapter(EuropeBaseAdapter):
    """Adapter for Imovirtual.com — Portugal's second-largest portal."""
    ...
    SEARCH_PARAMS = {
        ...
        "search[filter_enum_market]": "secondary",
    }
    ...
    def get_listing_urls(self) -> list[str]:
        ...
        while page <= 10:
            ...
            search_url = f"{self.base_url}/comprar/moradia/?{query}"
            ...
            for a in soup.select("a[href]"):
                href = a.get("href", "")
                if "/anuncio/" in href or "/imovel/" in href:
    ...
    def extract_listing(self, url: str) -> Optional[RawListing]:
        ...
        # Listing ID
        id_match = re.search(r'/(?:anuncio|imovel)/([^/]+)', url)
        ...
        building_sqm = None
        land_sqm = None
        for el in soup.select("[class*='detail'], [class*='feature'], li"):
            text = el.get_text()
            area = parse_area_sqm_europe(text)
            if area:
                if "terreno" in text.lower() or "land" in text.lower():
                    land_sqm = area
                elif not building_sqm:
                    building_sqm = area
        ...
        if t_match:
            ...
            m = re.search(r'(\d+)\s*(?:quarto|assoalhada)', text, re.IGNORECASE)
            if m:
                rooms = m.group(1)
